function [ data, g, data0 ] = air3Dhomework3(accuracy)
...
while(tMax - tNow > small * tMax)
  ...
  h = visualizeLevelSet(g, data, displayType, level, [ 'Radius = ' num2str(radius) ]);
...
nameStr = [ 'HyAvoid' num2str(radius) '.eps' ];
export_fig(nameStr);
...
hamValue = -(deriv{1} .* (-schemeData.cA + schemeData.cB * cos(schemeData.psi) + schemeData.oA * grid.xs{2})  ...
            +deriv{2} .* ( schemeData.cB * sin(schemeData.psi) - schemeData.oA * grid.xs{1}) ...
            );
...
switch dim
  case 1
    alpha = abs(-schemeData.cA + schemeData.cB * cos(schemeData.psi) + schemeData.oA .* grid.xs{2});

  case 2
    alpha = abs(schemeData.cB * sin(schemeData.psi) - schemeData.oA .* grid.xs{1});
